...
from berlin_urban_intelligence.adapters.vbb_static import (
    GtfsStaticSnapshot,
    VbbGtfsStaticAdapter,
    VbbGtfsStaticClient,
)
...
class ReferenceAcquisitionCoordinator:
    def __init__(
        self,
        *,
        wfs: WfsCoordinator | None = None,
        gtfs_client: GtfsReader | None = None,
        gtfs_adapter: VbbGtfsStaticAdapter | None = None,
        road_client: RoadReader | None = None,
    ...
    def refresh(
        self,
        *,
        previous: ReferenceState | None = None,
        include_gtfs: bool = False,
        include_osm: bool = False,
        place: str = "Berlin, Germany",
        network_type: str = "drive",
        allow_speed_imputation: bool = False,
    ) -> ReferenceState:
        ...
        if include_gtfs:
            try:
                snapshot: GtfsStaticSnapshot = self.gtfs_adapter.parse(
                    self.gtfs_client.fetch(), retrieved_at=now
                )
        ...
        if include_osm:
            try:
                ...
                road = road_client.fetch(
                    place,
                    network_type=network_type,
                    allow_speed_imputation=allow_speed_imputation,
                    retrieved_at=now,
                )
