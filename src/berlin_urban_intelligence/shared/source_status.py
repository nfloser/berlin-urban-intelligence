...
class SourceStatusStore:
    ...
    def record_success(self, source_id: str, *, retrieved_at: datetime, observation_time: datetime | None) -> None:
        ...
        self._state[source_id] = {"availability": AvailabilityStatus.AVAILABLE, "last_retrieval_attempt": retrieved, "last_successful_retrieval": retrieved, "latest_observation_time": observed, "error_code": None}
    ...
    def get(self, source_id: str, *, now: datetime, freshness_threshold: timedelta) -> SourceRuntimeStatus:
        ...
        return SourceRuntimeStatus(source_id=source_id, availability=current.get("availability", AvailabilityStatus.UNKNOWN), freshness=freshness, last_retrieval_attempt=current.get("last_retrieval_attempt"), last_successful_retrieval=current.get("last_successful_retrieval"), latest_observation_time=observed, error_code=current.get("error_code"))
