...
class ScenarioEngine:
    def apply_temperature_delta(self, baseline: Observation, scenario: Scenario) -> ScenarioTemperatureResult:
        ...
        return ScenarioTemperatureResult(
            id=f"scenario:{scenario.name}:{baseline.id}", baseline_id=baseline.id,
            scenario_name=scenario.name, phenomenon=baseline.phenomenon,
            value=float(baseline.value) + scenario.temperature_delta_c, unit=baseline.unit,
        )

    def apply_energy_demand_delta(self, baseline: Forecast, scenario: Scenario) -> ScenarioEnergyDemandResult:
        ...
        return ScenarioEnergyDemandResult(
            id=f"scenario:{scenario.name}:{baseline.id}", baseline_id=baseline.id,
            scenario_name=scenario.name, phenomenon=baseline.phenomenon,
            value=baseline.value * multiplier, unit=baseline.unit,
        )
