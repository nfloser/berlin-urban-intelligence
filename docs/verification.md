...
This matrix was established from current repository code, tests, documentation and GitHub Actions while addressing issue #11 on 2026-09-15.
...
| Provenance retained across canonical values | **PASS** | `Provenance` contract; adapter/data-pipeline tests; RDF projection; map canonical detail inspector; `docs/data/provenance.md`. | Semantic API completeness is tracked separately in #12. |
...
| Automatic runtime/reference/energy/derived hot reload | **PASS** | `_ApiStateController` watches all four persisted states before requests; frontend snapshot-token polling; API regression tests. | Compatibility documentation drift found by this audit must remain corrected. |
...
| Registry-driven deterministic workflow resolution | **PASS** | `orchestrator/engine.py` resolves `_WORKFLOW_CAPABILITIES` through `AgentRegistry` and dependency order; `tests/test_orchestrator.py`, `tests/test_agent_registry.py`. | Some compatibility docs still describe obsolete `_PLANS`; documentation drift is corrected with this audit. |
...
| Semantic RDF projection of canonical state | **PASS** | `knowledge/graph.py`, ontology/shapes, `tests/test_knowledge_graph.py`, `scripts/validate_knowledge.py`. | Full API graph/relationship use is separate below. |
| Derived lineage in semantic projection | **PARTIAL** | `knowledge/derived_graph.py` projects derivation definitions/records and PROV lineage; `refresh_live.py` includes it in generated live RDF. | `/api/v1/graph` currently builds graph from runtime/reference/energy only and there is no bounded general relationship-query endpoint. #12. |
| Semantic relationships used as an inspectable/queryable API capability | **PARTIAL** | RDF projection and derived dependency JSON exist. | Add typed semantic incoming/outgoing relationship queries and prove derived API graph parity in #12. |
...
| API exposes system/source/agent/domain/reference/scenario/derived/dependency/map surfaces | **PASS** | FastAPI routes and OpenAPI generation gate; API/map/derived tests; bounded list/map endpoints. | Semantic relationship completeness remains #12. |
...
| Documentation/reproducibility consistency | **PARTIAL** | Extensive primary docs, deterministic commands, source registry, architecture/evaluation/research sections exist. | Audit found compatibility/primary pages with stale restart/orchestrator statements; corrected in issue #11 before merge. Verification matrix must remain linked from docs index/status. |
...
The current overall gate is **NOT READY**. The following issues represent the known evidence gaps discovered by this audit:

- #12 — complete semantic API projection and relationship queries;
