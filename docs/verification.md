...
The permanent CI contract is `.github/workflows/ci.yml`. Live-source compatibility is intentionally separated into `.github/workflows/live-source-smoke.yml`; reference-source compatibility has a separate manual workflow, and optional real OSM routing readiness has its own road-network smoke workflow.
...
| No fabricated production measurements/fallbacks | **PASS** | Explicit unavailable/unknown semantics; `scripts/check_production_data.py`; deterministic fixtures are CI-only; runtime/source tests and docs. Road-network smoke failure evidence explicitly records `synthetic_fallback=false`. | Continue enforcing on future integrations. |
...
| Resilience routing/accessibility and immutable disruption overlays | **PASS** | `agents/resilience.py`; parallel-edge, snapping and scenario tests; Playwright baseline 100 s → disrupted 160 s acceptance fixture; road-readiness verifier reuses the real resilience router for the live acceptance contract. | Scientific/operational quality remains conditional on successful real road/reference acquisition (#14). |
...
| Dashboard provenance/derived/platform inspector implementation | **PASS** | `DerivedInspector`, `PlatformInspector`, canonical reference inspector and source/agent endpoints exist; PR #28 adds stable canonical source/agent row identifiers and deployment-safe source-registry configuration; composed Playwright acceptance verifies populated derived/source/agent/reload content. | Re-test when inspector contracts change. |
| End-to-end browser acceptance of principal workflow chain | **PASS** | PR #28 run `34989105040` passes all 6/6 Playwright cases through nginx → FastAPI → persisted `reference.json`/`state.json`/`derived.json` → React/MapLibre. Coverage includes hypothetical assessment, populated derived lineage, source availability/freshness/error semantics, agent health/capabilities/dependencies, reload diagnostics, deterministic workflow degradation, viewport/detail provenance, fail-closed map reload and baseline-vs-disruption routing. | Expand only when new principal workflows become part of v1 scope. |
...
| Documentation/reproducibility consistency | **PASS** | Primary docs, deterministic commands, source registry, architecture/evaluation/research sections and verification matrix are present; issue #11 corrected stale restart/orchestrator claims, issue #12 documents the semantic API contract, issue #13 documents the agent composition boundary, #14 has explicit road-network live evidence/blocked-state documentation, and #15 records populated persisted inspector acceptance plus deployment-safe source-registry configuration. | Maintain documentation in the same feature PR when contracts/behavior change. |
...
Issue #15 is no longer an open evidence gap: populated derived/source/agent/reload inspector behavior is verified end to end by PR #28. These issues remain intentionally separate; passing one must not silently change another gate to PASS.
